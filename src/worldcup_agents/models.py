...
class AgentConstitution(BaseModel):
    """One model's self-written betting constitution, public on its agent page.

    This is durable persona/accountability material. It is generated by the model itself
    and fed only to that same model during the bet step; it is not shared with competitors.
    """
    ...
    principles: list[str]
    aggression: str
    favorite_tolerance: str
    draw_appetite: str
    contrarian_tendency: str
    bankroll_discipline: str
    constitution: str


class AgentMemory(BaseModel):
    """Private self-memory for one competitor.

    The engine injects this only into the same model's future bet prompts. It is not public
    and deliberately excludes other agents' picks or reasoning, avoiding copy pressure.
    """
