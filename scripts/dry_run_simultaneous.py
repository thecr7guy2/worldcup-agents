...
RESULTS = {9001: (0, 1), 9002: (0, 1), 9003: (1, 0), 9004: (1, 0)}

# Canned bet decision per (competitor, fixture): (pick, stake). "pass" = sit it out.
#   AGGRO  fires near the 25% cap on all four  -> exposure climbs toward 100%.
#   PICKY  bets one match only                 -> disciplined, low exposure.
#   ROPES  starts the day already short ($40k)  -> dips to the floor mid-batch, then recovers.
DECISIONS = {
    AGGRO: {9001: ("home", 250_000), 9002: ("home", 250_000),
            9003: ("home", 250_000), 9004: ("home", 200_000)},
    PICKY: {9001: ("pass", 0), 9002: ("away", 150_000),
            9003: ("pass", 0), 9004: ("pass", 0)},
    ROPES: {9001: ("home", 10_000), 9002: ("home", 10_000),
            9003: ("away", 10_000), 9004: ("home", 10_000)},
}
START_BANKROLL = {ROPES: 40_000.0}  # others keep the seeded $1,000,000
...
def _seed(path: Path):
    ...
    for fid, home, away, grp, oh, od, oa in FIXTURES:
        db.upsert_fixture(conn, Fixture(id=fid, stage=Stage.GROUP, group=grp, kickoff=KO,
            home_id=name2id[home], away_id=name2id[away], status=MatchStatus.SCHEDULED))
        db.upsert_odds_snapshot(conn, OddsSnapshot(fixture_id=fid, captured_at=NOW,
            bookmaker="consensus", home=oh, draw=od, away=oa))
    for name, bankroll in START_BANKROLL.items():
        conn.execute("UPDATE competitor SET bankroll = ? WHERE model_name = ?",
                     (bankroll, name))
    conn.commit()
    return conn
...
def _stub_complete(decisions_for_model):
    ...
    def fake(model_id, prompt, **kw):
        ...
        captured[fid] = prompt
        pick, stake = decisions_for_model[fid]
        return (f'{{"pick": "{pick}", "stake": {stake}, "reasoning": "canned"}}',
                ModelCall(model_name=kw.get("model_name"), step="bet",
                          fixture_id=fid, created_at=NOW))
...
def main() -> None:
    ...
    print(f"  Per-match cap: {MAX_STAKE_FRACTION:.0%} of bankroll · bust floor "
          f"${BANKRUPT_FLOOR:,.0f}.")
    print("  Competitors:")
    for name in models:
    ...
    try:
        for fid, home, away, grp, oh, od, oa in FIXTURES:
            print(f"-- Match {fid}  (Group {grp})  {home} vs {away} "
                  f"[odds {oh}/{od}/{oa}] --")
            for name, model in models.items():
                pred = Prediction(model_name=name, fixture_id=fid, winner=Outcome.HOME,
                                  confidence=0.60, reasoning="canned", created_at=NOW)
                ...
                b = predict.bet(conn, model, fx[fid], pred, odds, bankroll,
                                home, away, force=True)
    ...
    for e in db.list_bankroll_history(conn, ROPES):
        if e.reason == "bet_settled":
            flag = "  ← at/below the bust floor!" if e.balance_after <= BANKRUPT_FLOOR else ""
            if flag:
                floor_touched = True
            print(f"   match {e.fixture_id}:  {e.delta:>+12,.0f}  →  balance "
                  f"${e.balance_after:>12,.0f}{flag}")
    final = db.get_competitor(conn, ROPES)
    print()
    if floor_touched and final.lives_used == 0:
        print(f"   {ROPES} touched the ${BANKRUPT_FLOOR:,.0f} floor MID-batch, but the bust")
        print("   check runs ONCE over the day's total — final balance "
              f"${final.bankroll:,.0f}, 0 lives spent.")
        print("   Per-fixture settling would have triggered a re-buy at that dip and burned")
