...
def main() -> None:
    ...
    def inspect_bet_prompt(prompt: str) -> None:
        ...
        assert "5% is still a meaningful bet" in prompt
        assert "There is no default tier" in prompt
        assert "MATCHDAY PORTFOLIO" in prompt
        assert "target allocation is 15%" in prompt
